...
const spriteColor = require('./gulp/tasks/sprite.js').spriteColor
...
const watcher =() => {
  ...
  // gulp.watch(pathes.watch.favicons, favicons);
  gulp.watch(pathes.watch.spriteicons, sprite);
  gulp.watch(pathes.watch.spriteColor, spriteColor);
  gulp.watch(pathes.watch.js, js)
}

//параллельные задачи
const show = gulp.parallel(watcher, server);
const mainTasks = gulp.parallel(copy, scss, images, html, sprite, spriteColor, ico, js)
