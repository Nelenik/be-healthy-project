export function ModalConstructor(triggerSelectorOrEl, userOptions) {
  this.core = {
    ...
    manageModal() {
      const {autoOpen, modalCloseBtnClass, modalOverlayClass, modalWrapperClass, closeWithEscape, closeClickingOnOverlay} = this.options;
      ...
      document.addEventListener('click', function (e) {
        ...
        if (this.isOpen && (e.target.closest(`.${modalCloseBtnClass}`) || !e.target.closest(`.${modalWrapperClass}`) && e.target.matches(`.${modalOverlayClass}`))) {
          if(!closeClickingOnOverlay) return
          this.closeModal();
        }
      }.bind(this))

      document.addEventListener('keydown', function (e) {
        if (this.isOpen && e.code == 'Escape' && closeWithEscape) this.closeModal();
      }.bind(this))
    },
